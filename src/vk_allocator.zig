...
pub fn wrap(allocator: *std.mem.Allocator) ?vk.AllocationCallbacks {
    if (allocator == std.heap.c_allocator) {
        return null;
    }
...
pub fn unwrap(callbacks: ?*const vk.AllocationCallbacks) *std.mem.Allocator {
    if (callbacks == null) {
        return std.heap.c_allocator;
    }
    return @ptrCast(*std.mem.Allocator, @alignCast(@alignOf(std.mem.Allocator), callbacks.?.p_user_data));
}
...
fn realloc(user_data: *c_void, original: ?*c_void, new_size: usize, calign: usize, _: vk.SystemAllocationScope) callconv(vk.vulkan_call_conv) ?*c_void {
    ...
    var mem = allocator.reallocBytes(
        old_mem,
        old_align,
        if (new_size > 0)
